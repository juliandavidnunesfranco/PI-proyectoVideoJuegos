...
const getVideogames = async () => {
    ...
    while (videoGames.length < 100) {
            ...
            let response = await axios.get(`https://api.rawg.io/api/games?key=${API_KEY}&page=${page}`) //  la variable response es la que trae todo
...
const getAllVideogames = async () => {
    ...
    let dbInfo = dbInfoP.map(el => {        //  dbInfo --->> se mapea adicionando mas features 
        return {
            id: el.id,
            name: el.name,
...
const getVideogameById = async (id) => {   // function que tiene como parametro un id
    if(id.length < 7){
        let apiUrl = await axios.get(`https://api.rawg.io/api/games/${id}?key=${API_KEY}`);  // segundo endpoint de la api externa
...
router.post('/videogames', async (req, res) => {
    try {
        ...
        res.send('Videogame successfully created!');  /// cambios
    } catch (error) {
        console.log(error)  
        // res.send(error)
    }
